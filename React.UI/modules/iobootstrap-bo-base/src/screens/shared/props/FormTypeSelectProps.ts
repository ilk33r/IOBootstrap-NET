...
class FormTypeSelectProps implements FormType {
    ...
    static initializeWithChangeListener(name: string, value: string, isEnabled: boolean, options: FormDataOptionModel[], validations: ValidationRule[], changeHandler: FormTypeChangeHandler): FormType {
        let response = new FormTypeSelectProps();
        response.name = name;
        response.value = value;
        response.isEnabled = isEnabled;
